...
class StudentView:
    ...
    def create_personal_info_frame(self):
        ...
        # Create info frame for student details
        info_frame = tk.Frame(self.main_container, bg="white")
        info_frame.pack(side=tk.LEFT, fill=tk.BOTH, padx=(0, 200))  # Added padding to separate from image
        ...
        for i, (label, value) in enumerate(self.labels):
            tk.Label(info_frame, text=label, font=("Times New Roman", 12, "bold"), 
                    bg="white").grid(row=i, column=0, sticky="w", padx=10, pady=5)
            label_value = tk.Label(info_frame, text=value, font=("Times New Roman", 12), 
                    bg="white")
            label_value.grid(row=i, column=1, sticky="w", padx=10, pady=5)
            self.value_labels.append(label_value)

        # Create image frame
        image_frame = tk.Frame(self.main_container, bg="white")
        image_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)

        # Image section
        img_path = r"D:\University\Exercise\PYTHON\python-T2\dataset\image_student\image.png"
        
        # Create a frame for the image border
        border_frame = tk.Frame(image_frame, bg="black", bd=1, relief="solid")
        border_frame.pack(anchor="n", pady=20)
        ...
        if os.path.exists(img_path):
            try:
                ...
                tk.Label(border_frame, text="Không thể tải ảnh", 
                        font=("Times New Roman", 12), bg="white", fg="red",
                        width=20, height=10).pack()
        else:
            tk.Label(border_frame, text="Ảnh sinh viên\nKhông tìm thấy ảnh", 
                    font=("Times New Roman", 12), bg="white", fg="#666666",
                    width=20, height=10).pack()

        # Add buttons frame at the bottom
        button_frame = tk.Frame(frame, bg="white")
        button_frame.pack(fill=tk.X, pady=150)

        # Create buttons with center alignment
        button_container = tk.Frame(button_frame, bg="white")
        button_container.pack(expand=True)

        self.edit_button = tk.Button(button_container, text="Sửa", 
                                   font=("Times New Roman", 12),
