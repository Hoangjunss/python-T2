...
def save(teacher: Teacher):
    db = Database()
    query = "INSERT INTO teacher (id, fullname, gender, status, address, email, phone, department_id) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)"
...
def update(teacher: Teacher):
    db = Database()
    query = "UPDATE teacher SET fullname=%s, gender=%s, status=%s, address=%s, email=%s, phone=%s, department_id=%s WHERE id=%s"
